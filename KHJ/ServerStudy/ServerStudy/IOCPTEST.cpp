...
int main(int argc, char *argv[]){
	...
	while (1){
		...
		printf("%ws \n", CLOCK.nowDayTime().c_str());					//접속 날짜와 시간
...
DWORD WINAPI WorkerThread(LPVOID arg)
{
	...
	while (1){
		...
		if (retval == 0 || cbTransferred == 0) {
			...
			printf("%ws, %ws \n", CLOCK.nowDayTime().c_str());						//종료 시간 날짜
			delete ptr;
			continue;
		}
		...
		if (ptr->recvbytes == 0) {
			...
			printf("[time : %ws / %s:%d] %s\n", CLOCK.nowTime().c_str(), inet_ntoa(clientaddr.sin_addr), ntohs(clientaddr.sin_port), ptr->buf);
		}
		else{
			ptr->sendbytes += cbTransferred;
		}
		if (ptr->recvbytes > ptr->sendbytes){//데이터 보냄
			ZeroMemory(&ptr->overlapped, sizeof(ptr->overlapped));
