...
{
	ZeroMemory(m_byKeyState, sizeof(BYTE) * MAX_KEY);
	ZeroMemory(&m_eMouseState, sizeof(DIMOUSESTATE));

	ZeroMemory(&m_bKeyDown, sizeof(bool)* MAX_KEY);
	ZeroMemory(&m_bKeyPressd, sizeof(bool)* MAX_KEY);
}
...
{
	m_pKeyBoardDevice->GetDeviceState(MAX_KEY, m_byKeyState);
